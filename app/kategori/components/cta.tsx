...
export default function CTA() {
  ...
  return (
    <section className="mx-auto max-w-7xl cursor-default px-6 pt-12 pb-24">
      <motion.article
        initial={{ opacity: 0, y: 40 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true }}
        transition={{ duration: 0.8, ease: "easeOut" }}
        className="bg-primary/5 relative flex flex-col items-center gap-10 overflow-hidden rounded-[2.5rem] px-8 py-12 lg:flex-row lg:px-12"
      >
        ...
        <div className="relative z-10 flex flex-1 flex-col items-start space-y-4 text-left">
          ...
          <p className="text-text-secondary max-w-lg text-sm leading-loose lg:text-base">
          ...
          <Link
            ...
            className="group mt-2 inline-flex w-full items-center justify-center gap-2 rounded-lg bg-[#25D366] px-7 py-4 font-bold text-white transition-transform hover:-translate-y-1 hover:shadow-lg hover:shadow-green-500/20 lg:w-fit"
          >
            <MessageCircle className="h-3 w-3 lg:h-5 lg:w-5" />
            <h5 className="text-xs lg:text-sm">Hubungi via WhatsApp</h5>
          </Link>
        </div>
