...
import { works } from "@/app/admin/karya/data/works";
  ...
  const rows = works.map((item) => [
    ...
    <article key="team" className="flex w-full flex-col gap-1">
      <span className="flex items-center gap-2 text-sm text-gray-700">
        <Calendar className="h-3.5 w-3.5 text-gray-400" /> {item.year}
      </span>
    ...
    <article key="link" className="w-40 truncate wrap-break-word">
      <Link href={item.link} className="text-xs text-blue-600 underline lg:text-sm">{item.link}</Link>
    </article>,
    <Badge key="status" className={`pointer-events-none border px-2.5 py-0.5 text-[10px] font-bold tracking-wider uppercase ${item.status === "Akan Datang" ? "border-green-200 bg-green-50 text-green-700 hover:bg-green-50" : item.status === "Progres" ? "border-red-200 bg-red-50 text-red-700 hover:bg-red-50" : "border-blue-200 bg-blue-50 text-blue-700 hover:bg-blue-50"}`}>
      {item.status}
    </Badge>,
  ...
  return (
    <>
      <div className="mx-auto mt-8 flex w-9/10 items-end justify-between gap-4 lg:w-19/20">
        <Table headers={["Info Karya", "Tim Produksi", "Tautan", "Status", "Aksi"]} rows={rows} sortable={["Status"]} />
      </div>
    </>
  );
}