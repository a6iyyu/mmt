...
import { ADMIN_COURSES_DETAIL, ADMIN_COURSES_EDIT, API_COURSES_DELETE } from "@/constants/route";
import type { course as Trainings } from "@/lib/generated/prisma/client";
  ...
  const rows = data.map((item) => [
    ...
    <article key="details" className="flex flex-col gap-1">
      <h5 className="flex items-center gap-2 text-xs text-gray-700">
        <Calendar className="h-3.5 w-3.5 text-gray-400" /> {formattedDate(item.date as Date)}
      </h5>
      <h5 className="flex items-center gap-2 text-xs text-gray-500">
        <Users className="h-3.5 w-3.5 text-gray-400" /> {item.mentor_name}
      </h5>
    </article>,
    <article key="quota" className="w-24 text-center">
      <h5 className="text-xs font-medium text-gray-700">
        {item.quota ? `${item.quota} Peserta` : "Tanpa Batas"}
      </h5>
    </article>,
    <Badge
      key="status"
      className={`pointer-events-none border px-2.5 py-0.5 text-[10px] font-bold tracking-wider uppercase ${
    ...
    >
      {item.availability === "OPEN" ? "DIBUKA" : item.availability === "FULL" ? "PENUH" : "TUTUP"}
    </Badge>,
    <article key="actions" className="flex justify-end gap-2">
      <Link href={ADMIN_COURSES_DETAIL(item.id)} className="rounded-lg p-2 text-gray-400 transition-colors hover:bg-gray-100 hover:text-blue-600">
        <FaCircleInfo className="h-4 w-4" />
      </Link>
      <Link href={ADMIN_COURSES_EDIT(item.id)} className="rounded-lg p-2 text-gray-400 transition-colors hover:bg-gray-100 hover:text-yellow-600">
        <Edit3 className="h-4 w-4" />
      </Link>
      <button onClick={() => handleDelete(item.id, item.title)} className="cursor-pointer rounded-lg p-2 text-gray-400 transition-colors hover:bg-gray-100 hover:text-red-600">
