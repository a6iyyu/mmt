...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  try {
    ...
    return {
      title: `Detail ${course.title} | Lab MMT`,
      description: course.description?.slice(0, 150) || "Detail Pelatihan Lab MMT",
    };
  } catch (error) {
  ...
  return (
    <DetailShell>
      <DetailHeader
        ...
        actions={
          <Link href={ADMIN_COURSES_EDIT(course.id)} className="flex items-center gap-2 rounded-lg bg-blue-600 px-6 py-3.5 text-xs font-bold text-white transition-all hover:bg-blue-700 hover:shadow-lg active:scale-95">
        ...
        <DetailAside>
          <figure className="relative overflow-hidden rounded-2xl border border-gray-100 bg-white p-2 shadow-sm">
            <div className="relative aspect-video w-full overflow-hidden rounded-xl bg-gray-100">
              ...
              <div className="absolute top-3 left-3">
                <span className={`rounded-full px-3 py-1 text-xs font-bold tracking-wider text-white uppercase shadow-sm backdrop-blur-sm ${course.availability === Availability.OPEN ? "bg-green-500/90" : "bg-red-500/90"}`}>
                  {course.availability.replace("_", " ")}
                </span>
              </div>
            </div>
          </figure>
          <InfoCard title="Informasi Singkat">
            <InfoItem
              ...
              label="Mentor"
              value={course.mentor_name || "Belum ditentukan"}
              iconColorClass="bg-purple-50 text-purple-600"
            />
            <InfoItem
              icon={<Users className="h-5 w-5" />}
              label="Kuota Peserta"
              value={`${course.quota || 0} Orang`}
              iconColorClass="bg-orange-50 text-orange-600"
            />
            <InfoItem
              icon={<MapPin className="h-5 w-5" />}
              label="Lokasi"
              value={course.location || "Belum ditentukan"}
          ...
          <section className="h-full rounded-2xl border border-gray-100 bg-white p-8 shadow-sm">
            <div className="mb-8 border-b border-gray-100 pb-8">
              ...
              <h5 className="flex items-center gap-2 text-sm text-gray-500">
                ...
                <time dateTime={course.date?.toISOString()}>
                  Tanggal Pelaksanaan:{" "}
                  {new Date(course.date as Date).toLocaleDateString("id-ID", {
                    weekday: "long",
                    year: "numeric",
                    month: "long",
                    day: "numeric",
