...
export default function LecturerList() {
  ...
  return (
    <section className="relative z-10 mx-auto flex w-[90%] max-w-7xl flex-col items-center pb-20">
      <motion.article className="flex w-full flex-col lg:items-center" variants={container} initial="hidden" animate="visible">
        ...
        <motion.p variants={item} className="mx-auto mt-4 max-w-2xl cursor-default text-sm leading-8 lg:text-center lg:text-base">
          Filter dosen berdasarkan bidang keahlian untuk menemukan pembimbing
          yang paling sesuai dengan minat penelitian Anda.
        </motion.p>
        <motion.div variants={item} className="no-scrollbar mt-6 mb-10 flex w-full max-w-full overflow-x-auto pb-2 lg:items-center lg:justify-center">
      ...
      <motion.div key={selectedField} className="grid grid-cols-1 gap-8 md:grid-cols-2 xl:grid-cols-3" variants={container} initial="hidden" animate="visible">
        {filteredLecturers.map((lecturer) => (
          <LecturerCard key={lecturer.id} lecturer={lecturer} variants={item} />
        ))}
      </motion.div>
