...
export function BenefitCard({ title, description, icon: Icon, backgroundImage, styling }: BC) {
  return (
    <motion.figure variants={card} initial="hidden" animate="visible" className={`relative h-60 w-full overflow-hidden rounded-2xl lg:h-full ${styling}`}>
      ...
      <figcaption className="relative z-10 flex h-full cursor-default flex-col justify-end p-6 text-white">
        <Icon className="text-accent mb-4 hidden h-9 w-9 lg:inline" />
