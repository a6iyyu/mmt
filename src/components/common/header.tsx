    ...
    <header className={`fixed top-0 z-50 w-full transition-all ${isScrolled ? "bg-white/80 shadow backdrop-blur dark:bg-neutral-900/80" : "bg-transparent"}`}>
      <div className="container mx-auto flex items-center justify-between px-10 py-5">
        <section className="flex items-center gap-3">
          <Image height={48} width={48} src="/images/jti.png" alt="JTI Polinema" className="h-8 w-8" loading="lazy" />
          <Image height={48} width={48} src="/images/console.png" alt="Game Developer Polinema" className="h-8 w-8" loading="lazy" />
          <h5 className="ml-2 hidden cursor-default font-semibold text-[#314158] lg:block dark:text-white">
            Lab Multimedia MMT
          </h5>
        </section>
      ...
      <nav className={`fixed top-0 left-0 h-screen w-full transform transition-transform duration-500 lg:hidden ${isOpen ? "translate-x-0" : "translate-x-full"} z-40 bg-white dark:bg-neutral-900`}>
        <section className="flex h-full flex-col px-6 py-6">
          ...
          {M.map((route, index) => (
            <Link key={index} href={`/${route.name.toLowerCase().replace(/ /g, "-")}`} className="group text-heading border-b-heading/50 flex items-center gap-3 border-b py-4 text-lg transition duration-300 last:border-b-0" onClick={() => setIsOpen(false)}>
              {route.icon}
              {route.name}
            </Link>
          ))}
